                ...
                <!-- Multiple Checkboxes -->
                    <div class="form-group">
                        <label class="col-md-4 control-label"
                               for="checkboxes">{!! str_replace('_',' ',ucfirst($option['name'])) !!}</label>
                        <div class="col-md-4">
                            <div class="checkbox">
        ...
        @foreach($options as $option)
            <div class="form-horizontal {{ $option['name'] }}  @if( !isset($data[$option['name']]['is_active']) || $data[$option['name']]['is_active']!=1 ) hidden  @endif">
                <fieldset>
                    ...
                    <div class="form-group">
                        <label class="col-md-4 control-label"
                               for="checkboxes">{!! str_replace('_',' ',ucfirst($option['name'])) !!} Options</label>
                        <div class="col-md-4">
                            @php
                                    ...
                                    @if(isset($item['type']) && $item['type'] == 'radio')
                                        <div class="radio">
                                            <label for="radios-{!! $item['slug'] !!}">
                                                <input type="radio"
                                                       name="{!! $option['name'] !!}[options][{!! $item['group'] !!}]"
                                                       @if(isset($data[$option['name']]['options'][$item['group']])
                                                       && $data[$option['name']]['options'][$item['group']] == $item['slug']) checked
                                                       @endif
                                                       id="radios-{!! $item['slug'] !!}" value="{!!  $item['slug'] !!}">
                                                {!! $item['name'] !!}
                                                - {!! str_replace('_',' ',ucfirst($item['slug'])) !!}
                                            </label>
                                        </div>
                                    @else
                                        <div class="checkbox">
                                            <label for="checkboxes-{!! $item['slug'] !!}">
                                                <input type="checkbox"
                                                       name="{!! $option['name'] !!}[options][{!! $item['slug'] !!}]"
                                                       @if(isset($data[$option['name']]['options'][$item['slug']])) checked
                                                       @endif
